...
def circo_generate_test_submission_file(dataset_path: str, clip_model_name: str,
                                        preprocess: callable,
                                        submission_name: str, scales=None) -> None:
    """
    Generate the test submission file for the CIRCO dataset given the pseudo tokens
    Args:
        scales: Optional list of scale values to generate multiple submission files
    """
    ...
    if clip_model_name == 'ViT-g-14':
        clip_model, _, _ = open_clip.create_model_and_transforms('ViT-g-14', device=device, pretrained='/home/jinzhenxiong/Imagine-and-Seek/weights/CLIP-ViT-g-14-laion2B-s12B-b42K/open_clip_pytorch_model.bin')
    else:
    ...
    queryid_to_retrieved_images = circo_generate_test_dict(relative_test_dataset, clip_model, index_features,
                                                           index_names, args.nums_caption, scales=scales)

    submissions_folder_path = PROJECT_ROOT / 'data' / "test_submissions" / 'circo'
    submissions_folder_path.mkdir(exist_ok=True, parents=True)

    # If scales were provided, save multiple files
    if scales is not None and isinstance(queryid_to_retrieved_images, dict) and all(isinstance(k, (int, float)) for k in queryid_to_retrieved_images.keys()):
        for scale, result_dict in queryid_to_retrieved_images.items():
            scale_str = f"{scale:.1f}".replace('.', '_')
            filename = f"{submission_name}_scale_{scale_str}.json"
            with open(submissions_folder_path / filename, 'w+') as file:
                json.dump(result_dict, file, sort_keys=True)
            print(f"Saved submission for scale={scale} to {filename}")
    else:
...
def circo_generate_test_dict(relative_test_dataset: CIRCODataset, clip_model: CLIP, index_features: torch.Tensor,
                             index_names: List[str], nums_caption, scales=None) \
        -> Dict[str, List[str]]:
    """
    Generate the test submission dicts for the CIRCO dataset given the pseudo tokens
    Args:
        scales: Optional list of scale values to generate multiple submissions.
                If provided, returns dict of {scale: submission_dict}
    """
    ...
    if args.use_momentum_strategy:
        ...
        if args.use_debiased_sample:
            ...
            if args.with_aug:
                ...
                # If scales list is provided, compute multiple submissions
                if scales is not None:
                    result_dict = {}
                    for scale in scales:
                        scale_similarity = scale * similarity + (1 - scale) * robust_aug_similarity * similarity
                        sorted_indices = torch.topk(scale_similarity, dim=-1, k=50).indices.cpu()
                        ...
                        result_dict[scale] = queryid_to_retrieved_images
                    return result_dict
                else:
...
def main():
    if args.eval_type in ['LDRE-B', 'LDRE-L', 'LDRE-G']:
        ...
        if clip_model_name == 'ViT-g-14':
            clip_model, _, clip_preprocess = open_clip.create_model_and_transforms('ViT-g-14', pretrained='/home/jinzhenxiong/Imagine-and-Seek/weights/CLIP-ViT-g-14-laion2B-s12B-b42K/open_clip_pytorch_model.bin')
        else:
    ...
    # Generate scale list if multi-scale testing is enabled
    scales = None
    if args.test_multiple_scales:
        import numpy as np
        scales = np.arange(args.scale_start, args.scale_end + args.scale_step/2, args.scale_step).tolist()
        scales = [round(s, 2) for s in scales]  # Round to avoid floating point issues
        print(f"Testing multiple scales: {scales}")

    circo_generate_test_submission_file(args.dataset_path, clip_model_name, preprocess, args.submission_name, scales=scales)
